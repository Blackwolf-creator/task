...
CREATE TABLE "campaigns" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "campaigns_payout_non_negative" CHECK ("campaigns"."payout_per_1k_views" >= 0),
	CONSTRAINT "campaigns_budget_non_negative" CHECK ("campaigns"."total_budget" >= 0),
	CONSTRAINT "campaigns_valid_date_range" CHECK ("campaigns"."starts_at" < "campaigns"."ends_at"),
...
CREATE TABLE "submissions" (
	...
	CONSTRAINT "submissions_rejection_reason_check" CHECK (
        (
          "submissions"."status" = 'rejected'
          AND "submissions"."rejection_reason" IS NOT NULL
          AND length(trim("submissions"."rejection_reason")) > 0
        )
        OR
        (
          "submissions"."status" <> 'rejected'
          AND "submissions"."rejection_reason" IS NULL
        )
      )
);
--> statement-breakpoint
CREATE TABLE "submission_metrics" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "submission_metrics_submission_day_unique" UNIQUE("submission_id","captured_at"),
...
CREATE INDEX "campaigns_starts_at_idx" ON "campaigns" USING btree ("starts_at");--> statement-breakpoint
CREATE INDEX "submissions_campaign_status_idx" ON "submissions" USING btree ("campaign_id","status");--> statement-breakpoint
CREATE INDEX "submissions_creator_idx" ON "submissions" USING btree ("creator_id");--> statement-breakpoint
CREATE INDEX "submission_metrics_submission_captured_idx" ON "submission_metrics" USING btree ("submission_id","captured_at");